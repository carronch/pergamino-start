...
Save the fetched content to `~/LLM{{SLUG}}Brain/raw/web/<YYYY-MM-DD>-<slug>.md` (per the routing in `RESOLVER.md`).
...
For each:
- Fetch and save to `~/LLM{{SLUG}}Brain/raw/autoresearch/<YYYY-MM-DD>/<slug>.md`.
...
### 6. Main{{SLUG}}Brain theme-level check

In addition to the per-source Main{{SLUG}}Brain check ingest already did, run a theme-level check: search Main{{SLUG}}Brain for the theme's keywords. If the author has a belief or principle on this topic, **prepend** it to the theme page's `## Current thesis` block as the starting position, citing the Main{{SLUG}}Brain file.
...
- Any Main{{SLUG}}Brain tension staged.
...
- Editing Main{{SLUG}}Brain directly under any circumstances.
