...
**a. Index first.** Read `~/LLMVault/wiki/_index.md` for the relevant section. Often the answer is two clicks away.
...
**c. Summaries.** Search `~/LLMVault/wiki/summaries/` for the topic. Use whatever search tool is available (`grep`, `qmd`, `ripgrep`, the librarian's own tooling). Read the matching summaries' TL;DRs and key claims.

**d. Raw sources.** Only descend to `~/LLMVault/raw/` if the wiki layer is thin. Reading raw is expensive — use sparingly.

**e. MainVault.** Always check MainVault for relevant beliefs or principles. The author's own position is the truth-anchor.

### 3. Synthesize

Write a 2–4 paragraph answer. Every factual claim cites the source: `[Source: wiki/summaries/<slug>]` or `[Source: MainVault: beliefs/<slug>]`. Do not paraphrase without citing.

If MainVault has a position on the question, **lead with it**. The author's own writing is the truth-anchor; the wiki is supporting structure.

If sources contradict each other, **say so explicitly** — *"Source A says X; Source B says Y; the author's belief at MainVault: beliefs/<slug>.md aligns with A."*
...
- MainVault gaps (e.g., "you don't have a belief on this; it might be worth writing one").
...
If filing, write a `synthesis` page per the schema in `CLAUDE.md`. Stage it for author review at `outputs/mainvault-pending/syntheses-<slug>.md` if it should crystallize into MainVault, or write directly to `wiki/syntheses/` if it's a librarian-side synthesis.
...
- **MainVault is the truth-anchor.** Lead with the author's own position when it exists.
...
- It doesn't update MainVault, even if the answer suggests the author should. Stage a tray proposal at most.
