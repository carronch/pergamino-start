...
Trigger phrases (the operator runs this):
...
Output: `~/Documents/GitHub/pergamino-deployments/<client-slug>/intake.yaml` (or wherever the operator keeps client deployment configs).

## The interview — 8 questions

You (the LLM running this skill) ask the operator and the client's principal in sequence. Don't batch. Each answer informs the next question. Adapt phrasing per client.
...
- 5–25 → `unit: small`
- 26–100 → `unit: mid`
- 100+ → `unit: enterprise` — pause, this is outside pergamino's current SKU; flag for the operator
...
Capture verbatim. This is the **success metric** — the 90-day review benchmark the operator will reference. Examples that count:
...
After all 8 answers, you generate the scope plan as YAML. Show it to the operator + the client principal. They review and tweak. Then save to disk.

```yaml
# Generated by pergamino-intake on <date>
# Reviewed by: <operator + principal-name>
...
# === Success metric ===
success_metric: |
  <operator + principal write this together>
...
- Does not interview employees individually. Only the principal + the operator. Other employees onboard via the rollout, not the intake.
...
- Companion skill once intake is done: `interview-executive` (per-leader question banks; populates `~/Main{{SLUG}}Brain/leadership/`)
- Output consumed by: `pergamino-deploy` CLI (the per-client provisioning binary)
- Reviewer-agent library that gets customized: `templates/reviewer-agents/` in this kit
