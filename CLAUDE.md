...
Every file in `~/LLMVault/wiki/` and every staged file in `outputs/mainvault-pending/` carries YAML frontmatter at the top:
...
One per ingested source. Lives in `~/LLMVault/wiki/summaries/<slug>.md`. The slug matches the raw file's slug.
...
- `## Tension with truth` (optional) — if this source contradicts a MainVault position
- `## Open threads` — questions surfaced by this source
- `## Timeline` — bullet log of when this was ingested and updated

### `concept`

A framework, mental model, or pattern the author tracks. Lives in `~/LLMVault/wiki/concepts/<slug>.md`. Created when three or more sources have referenced the same underlying idea (the **rule of three** — don't promote a one-off mention to a concept page).
...
- `## How the author uses it` — connect to MainVault if a belief or principle bears on it
- `## Open threads`
...
A long-running topic the author researches across many sources. Lives in `~/LLMVault/wiki/themes/<slug>.md`. A theme is broader than a concept — it's an *ongoing inquiry*, not a single framework.

Standard sections:
- `# <Theme name>` (H1)
- `## Current thesis` — 2–3 sentences, *dated*. **Tentative.** If the author has a MainVault belief on this, prepend it as the starting position.
...
A piece of active work the author is doing. Lives in `~/LLMVault/wiki/projects/<slug>.md`. Distinct from themes (theme = research; project = building).
...
6. Possibly: a tray staging if this source contradicts a MainVault belief.
...
in `~/LLMVault/wiki/log.md`. Schema changes are rare but consequential — the catalog inherits the change going forward.
